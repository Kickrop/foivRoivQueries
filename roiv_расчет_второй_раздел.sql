...
-- 				count(case when x2::int = 2 or x2::int = 1 or x2 is null then 99 end) x2_total
			from (
				select *
				from
				get_google_table('https://docs.google.com/spreadsheets/d/e/2PACX-1vTqBEet15-vvXa7M-nJhh6jG8c2wnl3E5d6YnwJkdXO1tRIGln5K-l34M_j5qYKzwXPqkayQNe0Okxa/pub?gid=943972237&single=true&output=csv'::text) a(id text, str_name text, str_num text, x1 text)
...
select *
from (
	select str_num, 'Да' as val, x1_yes x1, x1_yes_cut x1_cut, '1' answ_id		
		from data
