with data as (
		...
		from (
			select
				...
				count(case when x2::int = 2 or x2::int = 1 or x2 is null then 99 end) x2_total
			from (
				select *
				from
...
select *
from (
	select str_num, 'Да' as val, x1_yes x1, x1_yes_cut x1_cut, x2_yes x2, x2_yes_cut x2_cut, '1' answ_id		
		from data
	union all select str_num, 'Нет' as val,  x1_no, x1_no_cut, x2_no, x2_no_cut, '2' answ_id		
		from data
	union all select str_num, 'Без ответа' as val,  x1_pusto, x1_pusto_cut,  x2_pusto, x2_pusto_cut, '3' answ_id
		from data 
	union all select str_num, 'Всего' as val, x1_total, x1_total_cut, x2_total, x2_total_cut, '4' answ_id
		from data 
	) d3
order by str_num::int, answ_id